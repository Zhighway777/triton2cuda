...
__global__ void add_kernel(float* x_ptr, float* y_ptr, float* output_ptr, int n_elements) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("add_kernel", &add_kernel, "Add two tensors");
}
...
int main() {
    ...
    // 启动kernel: add_kernel
    add_kernel<<<numBlocks, blockSize>>>(d_a, d_b, d_result, N);
